...
function App() {
  ...
  let [chatDrafts, setChatDrafts] = useState({})
  ...
  useEffect(() => {
    fetchMeetups()
    let savedUser = localStorage.getItem('campusUser')
    if (savedUser) {
  ...
  const handleAuthSubmit = function(e) {
    e.preventDefault()
    let endpoint = isLoginView ? '/api/login' : '/api/register'
  ...
  if (!user) {
    if (showLanding) {
      let upcomingPreview = meetups.slice(0, 3)
      return (
        <div style={{ backgroundColor: '#121212', color: '#ffffff', minHeight: '100vh', fontFamily: 'sans-serif' }}>
          <header style={{ display: 'flex', justifyContent: 'space-between', padding: '20px 40px', alignItems: 'center', borderBottom: '1px solid #222' }}>
            ...
            <button onClick={() => setShowLanding(false)} style={{ backgroundColor: 'transparent', color: '#fff', border: '1px solid #fff', padding: '8px 25px', borderRadius: '20px', cursor: 'pointer', fontWeight: 'bold', transition: '0.2s' }}>
              Log In
            </button>
          </header>
          ...
          <div style={{ textAlign: 'center', padding: '80px 20px', maxWidth: '800px', margin: '0 auto' }}>
            ...
            <div style={{ display: 'flex', gap: '20px', justifyContent: 'center' }}>
              <button onClick={() => { setIsLoginView(false); setShowLanding(false); }} style={{ backgroundColor: '#1db954', color: '#000', border: 'none', padding: '15px 35px', borderRadius: '30px', fontSize: '1.1rem', fontWeight: 'bold', cursor: 'pointer', boxShadow: '0 4px 20px rgba(29, 185, 84, 0.4)' }}>
                Join the Community
              </button>
            </div>
          </div>
              ...
              <div style={{ display: 'flex', gap: '20px', justifyContent: 'center', flexWrap: 'wrap', maxWidth: '1200px', margin: '0 auto' }}>
                {upcomingPreview.map(meetup => (
                  <div key={meetup._id} style={{ backgroundColor: '#252525', padding: '20px', borderRadius: '12px', width: '300px', boxShadow: '0 8px 24px rgba(0,0,0,0.2)' }}>
                    ...
                    <button onClick={() => setShowLanding(false)} style={{ width: '100%', backgroundColor: '#333', color: '#fff', border: 'none', padding: '12px', borderRadius: '8px', cursor: 'pointer', fontWeight: 'bold', transition: '0.2s' }}>
                      View Details
                    </button>
                  </div>
                ))}
          ...
          <footer style={{ textAlign: 'center', padding: '60px 20px 40px', backgroundColor: '#1a1a1a', borderTop: '1px solid #222' }}>
            ...
            <button onClick={() => { setIsLoginView(false); setShowLanding(false); }} style={{ backgroundColor: '#1db954', color: '#000', border: 'none', padding: '15px 35px', borderRadius: '30px', fontSize: '1.1rem', fontWeight: 'bold', cursor: 'pointer', marginBottom: '40px' }}>
              Create Your Account
            </button>
    ...
    return (
       <div style={{ backgroundColor: '#121212', color: '#ffffff', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'sans-serif', flexDirection: 'column', position: 'relative' }}>
        <button onClick={() => setShowLanding(true)} style={{ position: 'absolute', top: '30px', left: '30px', backgroundColor: 'transparent', color: '#aaa', border: 'none', cursor: 'pointer', fontSize: '1rem', fontWeight: 'bold' }}>
          ← Back to Home
        </button>
  ...
  let joinedCount = meetups.filter(m => m.attendees && m.attendees.some(id => id.toString() === user.id.toString())).length
  let hostedCount = meetups.filter(m => m.creatorId === user.id).length
  let myUpcomingRSVPs = meetups.filter(m => m.attendees && m.attendees.some(id => id.toString() === user.id.toString())).slice(0, 3)
  
  let filteredMeetups = meetups.filter(m => {
    let safeTitle = m.title || ""
    let safeDescription = m.description || ""
    let safeTags = m.tags || ""
    let matchesSearch = safeTitle.toLowerCase().includes(searchTerm.toLowerCase()) || 
                          safeDescription.toLowerCase().includes(searchTerm.toLowerCase()) ||
                          safeTags.toLowerCase().includes(searchTerm.toLowerCase())
    let matchesCategory = activeFilter === 'All' || m.category === activeFilter
      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', gap: '25px', alignItems: 'flex-start' }}>
          ...
          <div style={{ flex: '2', display: 'flex', flexDirection: 'column', gap: '20px' }}>
            <div style={{ backgroundColor: '#1e1e1e', padding: '20px', borderRadius: '12px', boxShadow: '0 4px 20px rgba(0,0,0,0.2)' }}>
              {!isFormExpanded ? (
                <div onClick={() => setIsFormExpanded(true)} style={{ backgroundColor: '#333', padding: '15px', borderRadius: '30px', color: '#aaa', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '10px' }}>
                ...
                <form onSubmit={handleCreateMeetup} style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                  ...
                  <input type="file" accept="image/*" onChange={(e) => setCoverImageFile(e.target.files[0])} style={{ padding: '12px', borderRadius: '8px', border: 'none', backgroundColor: '#333', color: 'white' }} />
            ...
            <div>
              {filteredMeetups.length === 0 ? (
                ...
                filteredMeetups.map((meetup) => {
                  let attendeeStrings = meetup.attendees ? meetup.attendees.map(id => id.toString()) : []
                  let isAttending = attendeeStrings.includes(user.id.toString())
                  ...
                  <div key={meetup._id} className="hover-card" style={{ backgroundColor: '#1e1e1e', borderRadius: '12px', overflow: 'hidden', marginBottom: '20px', boxShadow: '0 4px 20px rgba(0,0,0,0.2)' }}>
                    ...
                    <div style={{ width: '100%', height: '200px', backgroundColor: '#333', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden' }}>
                      <img src={meetup.coverImage || 'https://undraw.co/api/images?path=images/illustrations/group_hangout_v86q.svg'} alt="Event Cover" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                    </div>
                    ...
                    <div style={{ padding: '24px' }}>
                      ...
                      <button onClick={() => handleJoinMeetup(meetup._id)} style={{ backgroundColor: isAttending ? '#333' : '#ffffff', color: isAttending ? '#ffffff' : '#000000', border: isAttending ? '1px solid #555' : 'none', padding: '12px 24px', borderRadius: '30px', fontWeight: 'bold', cursor: 'pointer', marginBottom: '20px', transition: 'all 0.2s ease', width: '100%' }}>
